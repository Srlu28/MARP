...
protected:
   ...
   /*
    ...
    nullptr si el hijo es vacío, la altura y el número de 
    elementos en el hijo izquierdo más 1.
    */
   struct TreeNode;
   using Link = TreeNode *;
   // Modificamos el constructor y los atributos de TreeNode.
   struct TreeNode {
      T elem;
      Link iz, dr;
      int altura, tam_i;
      TreeNode(T const& e, Link i = nullptr, Link d = nullptr,
               int alt = 1, int ti = 1) : elem(e), iz(i), dr(d), altura(alt), tam_i(ti) {}
   };
...
public:
   ...
   T const& kesimo(int k) const { // O(log n), siendo n el número de nodos del árbol
...
protected:
   ...
   bool inserta(T const& e, Link & a) {
      ...
      if (a == nullptr) { // se inserta el nuevo elemento e
         ...
         // Solo si hemso descendido por el hijo izquierdo 
         // y el árbol ha crecido sumamos uno a tam_i.
         if (crece) {
             ++a->tam_i;
             reequilibraDer(a);
         }
   ...
   T const& kesimo(int const k, Link a) const {
       // Si el elemento no está en el árbol
       if (a == nullptr) throw std::domain_error("There is no enough elements");

       // Si k es menor que los nodos en el hijo izquierdo,
       // continuamos la búsqueda del kesimo en él.
       if (k < a->tam_i) return kesimo(k, a->iz);

       // En caso contrario, dado que k es mayor, le restamos el
       // número de nodos en el hijo izquierdo para buscar el
       // kesimo en el subárbol derecho.
       if (k > a->tam_i) return kesimo(k - a->tam_i, a->dr);
   ...
   void rotaDer(Link & r2) {
      ...
      // Ajustamos el tamaño del subárbol izquierdo de r2 tras quitar r1 y
      // su subárbol izquierdo del subárbol izquierdo de r2.
   ...
   void rotaIzq(Link & r1) {
      ...
      r1->altura = std::max(altura(r1->iz), altura(r1->dr)) + 1;
      r2->altura = std::max(altura(r2->iz), altura(r2->dr)) + 1;
      // Ajustamos el tamaño del subárbol izquierdo de r2 tras añadir
      // r1 y su subárbol izquierdo al subárbol izquierdo de r2.
      r2->tam_i += r1->tam_i;
      r1 = r2;
   }
